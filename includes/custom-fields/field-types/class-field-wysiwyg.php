 ...
 *	Custom Fields
 *
 * @package   	Custom Fields Class field - Checkbox
...
class CustomField_Wysiwyg extends CustomField {

	public $default_field_class = 'dev-wysiwyg';
	...
	public function display() {
		 ...
		 * @var string
		 */
		$wysiwyg = boolval( dev_get_option( 'frontend_wysiwyg_editor' ) );
		...
		if ( true === $wysiwyg || is_admin() ) {
			...
			$editor = "<label {{label_atts}}>{{label}}</label><div class='dev-submit-ticket-wysiwyg'>$editor</div>";

		} else {

			$path = DEV_PATH . "includes/custom-fields/field-types/class-field-textarea.php";
