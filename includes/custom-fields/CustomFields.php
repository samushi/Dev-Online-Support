...
use DevSupport\Field\CustomField;
...
class CustomFields {
	...
	public function __construct() {
		...
		if( is_admin() ) {
			...
			add_filter( 'dev_before_submit_new_ticket_checks', array( $this, 'check_required_fields' ) );

			/* Save the custom fields. */
			add_action( 'dev_open_ticket_before_assigned', array( $this, 'frontend_submission' ), 10, 2 );

			/* Display the custom fields on the submission form */
			add_action( 'dev_submission_form_inside_after_subject', array( $this, 'submission_form_fields' ) );
	...
	public function add_field( $name = '', $args = array() ) {
		...
		/* Default arguments */
		$defaults = CustomField::get_field_defaults();
		...
		if ( ! empty( $arguments['callback'] ) ) {
			...
			_deprecated_argument( 'CustomFields::add_field()', '1.0', sprintf( __( 'Please use %s to register your custom field type', TEXTDOMAIN ), '<code>field_type</code>' ) );
		...
		$this->options[ $name ] = apply_filters( 'dev_add_field', $option );
	...
	public function add_custom_column( $columns ) {
		...
		foreach ( $fields as $field ) {
			...
			if ( true === $field['args']['show_column'] ) {
				$id            = $field['name'];
				$title         = dev_get_field_title( $field );
				$custom[ $id ] = $title;
			}
	...
	public function status_filter_by_status( $query ) {
		...
		if ( is_admin()
		     ...
		     && isset( $_GET['dev_status'] )
		     && ! empty( $_GET['dev_status'] )
		     && $query->is_main_query()
		) {

			$query->query_vars['meta_key']     = '_dev_status';
			$query->query_vars['meta_value']   = sanitize_text_field( $_GET['dev_status'] );
	 ...
	 *
	 * This function is hooked on the filter dev_before_submit_new_ticket_checks
		...
		foreach ( $fields as $field_id => $field ) {
			...
			if ( true === $field['args']['required'] && false === $field['args']['core'] ) {
				...
				if ( ! isset( $data[ $field_name ] ) || empty( $data[ $field_name ] ) ) {
					...
					/* Get field title */
					$title = ! empty( $field['args']['title'] ) ? $field['args']['title'] : dev_get_title_from_id( $field['name'] );

					/* Add the error message for this field. */
					if ( ! is_object( $result ) ) {
						$result = new \WP_Error( 'required_field_missing', sprintf( __( 'The field %s is required.', TEXTDOMAIN ), "<a href='#$field_name'><code>$title</code></a>", array( 'errors' => $field_name ) ) );
					} else {
						$result->add( 'required_field_missing', sprintf( __( 'The field %s is required.', TEXTDOMAIN ), "<code>$title</code>", array( 'errors' => $field_name ) ) );
					}
