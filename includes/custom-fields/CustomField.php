...
class CustomField {
	...
	public static function get_field_defaults() {
		...
		$defaults = array(
			...
			'column_callback'       => 'dev_cf_value',       // Column callback function
	...
	public function get_class_name() {
		...
		if ( ! isset( $this->class_name ) ) {
			...
			$this->class_name = "DevSupport\Fields\Field\CustomField_$type";
		}
	...
	protected function require_field_type_class() {
		...
		$field_class_path = DEV_PATH . "includes/custom-fields/field-types/class-field-{$this->field['args']['field_type']}.php";
	...
	public function populate() {
		...
		if ( empty( $value ) ) {
			...
			$fields = DEVSUPPORT()->session->get( 'submission_form' );
	...
	protected function get_wrapper_markup() {
		...
		return apply_filters( 'dev_cf_wrapper_markup', $default, $this->field, $this->get_wrapper_class(), $wrapper_id );
	...
	protected function get_field_markup() {
		 ...
		 * Load user function.
		 *
		 * This method was deprecated in 1.0.0 but we keep it active
		 * for backward compatibility with custom fields created prior
		 * to this change.
		 *
		 * If you're creating custom fields with version 1.0.0, you should
			...
			if ( method_exists( $class_name, 'display' ) ) {
				...
				if ( is_admin() ) {
					if ( ! current_user_can( $this->field['args']['capability'] ) && method_exists( $instance, 'display_no_edit' ) ) {
						...
						$field = apply_filters( 'dev_cf_display_admin_markup', $instance->display_admin(), $this->field, $this->populate() );
					} else {
				...
				$field  = '<!-- ' . __( 'The custom field class does not contain the mandatory method "display"', TEXTDOMAIN ) . ' -->';
				$error = true;
			}
		...
		/* In case the field type / callback function does not exist */
			...
			$field  = '<!-- ' . __( 'The type of custom field you are trying to use does not exist', TEXTDOMAIN ) . ' -->';
			$error = true;
		}

		return false === $error ? $this->process_field_markup( apply_filters( 'dev_cf_field_markup', $field, $this->populate(), $this->field ) ) : $field;
	...
	protected function process_field_markup( $field = '' ) {
		...
		$label_class = apply_filters( 'dev_cf_field_label_class', $label_class, $this->field );
		...
		$field = str_replace( '{{atts}}', implode( ' ', apply_filters( 'dev_cf_field_atts', $atts, $field, $this->field ) ), $field );
		$field = str_replace( '{{label_atts}}', implode( ' ', $label_atts ), $field );
		$field = str_replace( '{{label}}', $this->get_field_label(), $field );

		return apply_filters( 'dev_cf_field_markup_processed', $field, $this->field );
	...
	public function get_wrapper_class( $class = array() ) {
		...
		if ( isset( $_SESSION['dev_submission_error'] ) && is_array( $_SESSION['dev_submission_error'] ) && in_array( $this->get_field_id(), $_SESSION['dev_submission_error'] ) ) {
			array_push( $classes, 'has-error' );
		}

		/* Filter the final list */
		$classes = apply_filters( 'dev_cf_wrapper_class', $classes, $this->field );
		...
		if ( ! empty( $class ) ) {
			 ...
			 * @var $erase bool
			 */
			$erase = apply_filters( 'dev_cf_wrapper_class_force_erase_extra', false );
	...
	public function get_field_class( $class = array() ) {
		...
		/* Filter the final list */
		$classes = apply_filters( 'dev_cf_field_class', $classes, $this->field );
		...
		if ( ! empty( $class ) ) {
			 ...
			 * @var $erase bool
			 */
			$erase = apply_filters( 'dev_cf_field_class_force_erase_extra', false );
	...
	public function get_field_description() {
		 ...
		 * @var $classes array
		 */
		$classes = apply_filters( 'dev_cf_field_description_class', array( 'backend'  => 'description', 'frontend' => 'dev-help-block' ), $this->field );
		...
		return apply_filters( 'dev_cf_description_markup', $description );
