 ...
 * @since  1.0.0
 */
function dev_nonce_url( $url ) {
	return add_query_arg( array( 'dev-nonce' => wp_create_nonce( 'dev_custom_action' ) ), $url );
}

/**
 * Add custom action and nonce to URL.
 *
 * The function adds a custom action trigger using the dev-do
 ...
 * @since  1.0.0
 */
function dev_get_option( $option, $default = false ) {
	...
	return apply_filters( 'dev_option_' . $option, $value );
 ...
 */
function dev_get_ticket_status( $post_id = null ) {
	...
	return get_post_meta( $post_id, '_dev_status', true );
 ...
 */
function dev_dropdown( $args, $options ) {
	...
	<select name="<?php echo $args['name']; ?>" <?php if ( !empty( $class ) ) echo 'class="' . implode( ' ' , $class ) . '"'; ?> <?php if ( !empty( $id ) ) echo "id='$id'"; ?> <?php if( true === $args['disabled'] ) { echo 'disabled'; } ?>>
		...
		if ( $args['please_select'] ) {
			echo '<option value="">' . __( 'Please select', TEXTDOMAIN ) . '</option>';
		}
 ...
 * @return bool
 */
function dev_update_option( $option, $value ) {
	...
	return update_option( 'dev_options', serialize( $options ) );
...
function dev_get_open_ticket_url( $ticket_id, $action = 'open' ) {
...
function dev_get_close_ticket_url( $ticket_id ) {
	return dev_get_open_ticket_url( $ticket_id, 'close' );
}
...
function dev_get_field_title( $field ) {

	if ( !empty( $field['args']['title'] ) ) {
		return sanitize_text_field( $field['args']['title'] );
	} else {
		return dev_get_title_from_id( $field['name'] );
	}
...
function dev_make_button( $label = null, $args = array() ) {
	...
	$defaults = array(
		'type'     => 'button',
		'link'     => '',
		'class'    => apply_filters( 'dev_make_button_class', 'dev-btn dev-btn-default' ),
		'name'     => 'submit',
		'value'    => '',
		'onsubmit' => ''
	);
 ...
 */
function dev_get_ticket_status_state( $post_id ) {
	...
	if ( 'closed' === $status ) {
		...
		$post          = get_post( $post_id );
		$post_status   = $post->post_status;
		$custom_status = dev_get_post_status();
...
function dev_get_current_admin_url() {
 ...
 */
function dev_redirect( $case, $location = null, $post_id = null ) {
	 ...
	 */
	$location = apply_filters( "dev_redirect_$case", $location, $post_id );
...
add_filter( 'locale','dev_change_locale', 10, 1 );
/**
 ...
 */
function dev_get_settings_page_url( $tab = '' ) {

	$admin_url  = admin_url( 'edit.php' );
	$query_args = array( 'post_type' => 'ticket', 'page' => 'dev-settings' );
...
if ( ! function_exists( 'dev_get_admin_path_from_url' ) ) {
	/**
	 * Get the admin path based on the URL.
	 *
	 * @return string Admin path
	 */
	function dev_get_admin_path_from_url() {
 ...
 * @return void
 */
function dev_hierarchical_taxonomy_dropdown_options( $term, $value, $level = 1 ) {
	...
	<?php if ( isset( $term->children ) && !empty( $term->children ) ) {
		...
		foreach ( $term->children as $child ) {
			dev_hierarchical_taxonomy_dropdown_options( $child, $value, $level );
		}
	}
 ...
 * @return string
 */
function dev_get_submission_page_url( $post_id = false ) {

	$submission = dev_get_submission_pages();
 ...
 * @return string
 */
function dev_get_tickets_list_page_url() {
 ...
 */
function dev_get_reply_link( $reply_id ) {
	...
	$replies = dev_get_replies( $reply->post_parent, array( 'read', 'unread' ) );
